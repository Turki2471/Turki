...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@YY133"))
...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@WL_LF"))
    except BaseException:
        pass  
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@QQRHR"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : اسمك"
    )
...
@ze.on(events.NewMessage(outgoing=True, pattern=r".اذاعة للقروبات"))
...
async def _(event):
    ...
    872014853 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"872014853 : `{event.sender_id}`")
    else:
